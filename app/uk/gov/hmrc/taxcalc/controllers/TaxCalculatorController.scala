...
trait TaxCalculatorController extends BaseController with ErrorHandling {
  ...
  def calculateTax(taxYear: Int, taxCode: String, grossPay: Long, payPeriod: String, journeyId: Option[String]) = Action.async {
    implicit request => {
      service.calculateTax(taxYear, taxCode, grossPay, payPeriod).map{
